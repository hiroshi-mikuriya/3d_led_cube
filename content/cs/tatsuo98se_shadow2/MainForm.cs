...
namespace hands_viewer.cs
{
    public partial class MainForm : Form
    {
        ...
        long freq, last;
        List<List<FixedSizedQueue<Point>>> fingerBuffer = new List<List<FixedSizedQueue<Point>>>();
        ...
        public MainForm(Session session)
        {
            ...
            var leftHand = new List<FixedSizedQueue<Point>>();
            var rightHand = new List<FixedSizedQueue<Point>>();
            for(int i=0; i<5; i++)
            {
                leftHand.Add(new FixedSizedQueue<Point>());
                rightHand.Add(new FixedSizedQueue<Point>());
            }
            fingerBuffer.Add(leftHand);
            fingerBuffer.Add(rightHand);
        }
...
//                                                HandsRecognition.addObject(new LED3DEphemeralDot(ledX, ledY, 0, color));
        ...
        private bool isHandDistanceGreaterThan(FixedSizedQueue<Point> finger, int d)
        {
            return isDistanceGreaterThan(finger.lastObject(), finger.lastlastObject(), d);
        }

        private bool isDistanceGreaterThan(Point pt1, Point pt2, int d)
        {
            var distance = Math.Pow(pt1.X - pt2.X, 2) + Math.Pow(pt1.Y - pt2.Y, 2);
//            Console.WriteLine("Point1: " + pt1 + " Point2: " + pt2 + " dis:" + distance);
            return distance > (d * d);
        }
