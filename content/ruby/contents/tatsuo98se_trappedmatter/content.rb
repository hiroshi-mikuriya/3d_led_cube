...
class IncrementalMultipleCircle < Dot

    def initialize(x, y, z, r, color, direction, size=3)
        super(x, y, z, color, -1)
        @first_r = r
        @rs = [r]
        @direction = direction
...
class TrappedMatter < Dot
    ...
    def draw(canvas)
        ...
        if @x > WIDTH or @x < 0 then
            @xadd = -@xadd
            @x += @xadd
            canvas.add_object(IncrementalMultipleCircle.new( 
                            @x, @y, @z, 0, RGB.rand(0xaa), 
                            DrawUtility::DIRECTION_SIDE))
        end
