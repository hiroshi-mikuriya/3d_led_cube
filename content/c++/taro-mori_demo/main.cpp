...
int main(int argc, const char* argv[])
{
    ...
    po::options_description desc("3D LED DEMO PROGRAM");
    desc.add_options()	//
        ("help,h", "produce help message")	//
        ("version,v", "produce version information")	//
        ("url,u", po::value<std::string>(), "3D LED URL")	//
        ("port,p", po::value<int>(), "3D LED PORT")	//
        ;
    po::variables_map vm;
    try
    {
        ...
        if (vm.count("version"))
        {
            std::cout << "version 1. 2018. 0" << std::endl;
            return 0;
        }
        if (vm.count("help"))
        {
        ...
        po::notify(vm);
        if (0 < vm.count("url") && 0 < vm.count("port")){
            std::string const url = vm["url"].as<std::string>();
            int const port = vm["port"].as<int>();
            std::cout << "URL : " << url << "\n" "PORT : " << port << std::endl;
            SetUrl(url.c_str());
            SetPort(port);
        }
